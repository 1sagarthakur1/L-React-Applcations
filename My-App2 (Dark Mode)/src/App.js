...
const [btntxt,setbtntxt] = useState("Enable to Dark mode")
...
const [myStyle,setStyle] = useState({
 ...
 const togglstyle = ()=>{
   if(myStyle.backgroundColor === 'white'){
    ...
    setbtntxt("Enable to Light mode")
   }else{
    setStyle({
      color:'gray',
      backgroundColor:'white'
    })
    setbtntxt("Enable to Dark mode")
   }
 }
  ...
  <div className="container my-3 ">
    <button type="button" onClick = {togglstyle} className="btn btn-danger">{btntxt}</button>
  </div>
  </div>
  </>
  );
